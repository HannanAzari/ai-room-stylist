...
import {
  ...
  trackConceptSelection,
  trackDownloadClicked,
  trackFavouriteClicked,
...
import type { Product, RoomStylistMode } from "../types";
...
export function RoomStylistApp({
  mode = "classic",
}: {
  mode?: RoomStylistMode;
}) {
  ...
  const [generateProgressIndex, resetGenerateProgressIndex] = useProgressIndex(
    loading,
    generateProgressMessages.length,
    3500
  );
  const [refineProgressIndex, resetRefineProgressIndex] = useProgressIndex(
    refining,
    refineProgressMessages.length,
    3000
  );
  ...
  const routeLabel = isStudio ? "Studio Pilot" : "AI Room Stylist";
  const routeSummary = isStudio
    ? "A mobile-first pilot workspace for room uploads, product references, AI concepts, refinement, and demo ecommerce actions."
    : "Curate a complete room concept with real furniture references, premium styling, and a shoppable bundle.";
  const headline = isStudio
    ? "Create a pilot-ready room concept"
    : "Redesign your room with luxury furniture";
  const introCopy = isStudio
    ? "Use the existing generation and refinement pipeline in a cleaner studio route built for the next pilot iteration."
    : "Upload a room photo, choose a style, and generate premium interior concepts with matching furniture suggestions.";
  ...
  async function shareGeneratedConcept(index: number) {
    ...
    trackShareClicked(index, shareMethod);
  ...
  async function handleGenerate() {
    ...
    trackGenerateStarted({
      ...
      style,
      selectedProductIds,
      roomMeasurements: roomMeasurementPayload,
    });
      ...
      >
        <div className="mb-10 border-b border-neutral-800 pb-8">
          <div className="flex flex-wrap items-end justify-between gap-6">
            ...
            <p className="max-w-sm text-left text-sm leading-6 text-neutral-400 sm:text-right">
              {routeSummary}
            </p>
          </div>
                    ...
                    <div className="grid gap-2">
                      <button
                        ...
                        onClick={() => {
                          ...
                          trackConceptSelection(isSelected, index);
                        }}
